...
public class S1Agent_Raycasts_FindDeliver : Agent
{
    ...
    private bool currentlyHoldsCorrectItem = false;
    ...
    private void EnterTrigger(Collider other)
    {
        if (items.Contains(other.gameObject))
        {
            if (other.gameObject == demandedItem)
            {
                ...
                currentlyHoldsCorrectItem = true;
        ...
        else if (other.gameObject == warehouse.gameObject)
        {
            if (currentlyHoldsCorrectItem)
            {
    ...
    {
        currentlyHoldsCorrectItem = false;
    ...
    private RaycastObservation[] GetRaycastObservations()
    {
        ...
        for (int i = 0; i < numberOfRays; i++)
        {
            ...
            raycastObservation.warehouse = false;
            // Debuging Lines
            if (blocked)
            {
                ...
                if (hit.collider.gameObject == warehouse.gameObject)
                {
                    raycastObservation.warehouse = true;
                    Debug.DrawLine(startPosition, hit.point, Color.gray);
                }
            }
            else
            {
    ...
    {
        // Adds one-hot encoded demanded items
        ...
        foreach (RaycastObservation obs in raycastsObservations)
        {
            ...
            sensor.AddObservation(obs.warehouse);
        }
    }
